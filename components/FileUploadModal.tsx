    ...
    >
      ...
      >
        <div className="p-5 border-b border-slate-700/50 flex justify-between items-center flex-shrink-0">
          <h3 className="text-lg font-bold text-white">{t('fileUploadModal.title', { serviceName: service.name })}</h3>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 space-y-6">
          <div>
            <h4 className="font-semibold text-slate-200 mb-3">{t('fileUploadModal.requiredDocuments')}</h4>
          ...
          <form onSubmit={handleSubmit}>
            <div 
              className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-300 ${isDragOver ? 'border-amber-500 bg-amber-500/10 scale-105' : 'border-slate-600 bg-slate-800/30 hover:border-amber-500/50'}`}
            ...
            >
              ...
              <p className="mt-2 text-sm text-slate-400">
                <span className="font-semibold text-amber-400">{t('fileUploadModal.dragDrop')}</span> {t('fileUploadModal.orClick')}.
              </p>
            </div>

            {files.length > 0 && (
              <div className="mt-4">
                <h5 className="font-semibold text-sm text-slate-300 mb-2">{t('fileUploadModal.selectedFiles')}</h5>
                  ...
                  {files.map(file => (
                    <li key={file.name} className="flex items-center justify-between bg-slate-700/50 p-2 rounded-md text-sm border border-slate-600/50">
                      ...
                      <button 
                        ...
                        aria-label={t('fileUploadModal.remove', { fileName: file.name })}
                      >
            ...
            <div className="mt-6">
              <button
                ...
                className="w-full bg-amber-600 text-white py-3 px-4 rounded-lg hover:bg-amber-700 disabled:bg-slate-600 disabled:cursor-not-allowed font-bold shadow-lg shadow-amber-900/40 hover:shadow-amber-700/50 transform hover:scale-105 transition-all"
              >
                {isUploading ? t('fileUploadModal.uploading') : t('fileUploadModal.upload', { count: files.length })}
              </button>
            </div>
          </form>
        </div>
