...
const ServiceCard: React.FC<{ service: typeof services[0] }> = ({ service }) => (
  <div className="glass-card p-6 rounded-xl flex flex-col h-full">
    ...
    <div className="flex justify-between items-center text-xs text-slate-400 border-t border-slate-700/50 pt-4">
      <span>
        <span className="font-semibold text-slate-200">الرسوم:</span> {service.fees}
      </span>
      <span className="font-semibold text-emerald-400">{service.time}</span>
    </div>
  </div>
);
...
const CustomSelect: React.FC<{
  ...
  return (
    <div ref={selectRef} className="relative w-full">
      <button
        type="button"
        className="dark-input w-full px-4 py-2.5 rounded-lg text-right flex justify-between items-center"
...
const ServicesPage: React.FC = () => {
  ...
  const agencies = useMemo(() => {
    ...
    return uniqueAgencies.map(agency => ({
      value: agency,
      label: agency === 'all' ? 'جميع الجهات' : agency,
    }));
  }, []);
  
  const filteredServices = useMemo(() => {
    return services.filter(service => {
      const matchesSearch = service.name.toLowerCase().includes(searchTerm.toLowerCase()) || service.agency.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div>
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold text-slate-100">دليل الخدمات</h1>
        <p className="text-slate-300 mt-2">تصفح وابحث في جميع الخدمات المتاحة للمستثمرين في المملكة.</p>
      </div>
